...
# --- 3. المعالجة الحسابية (تتم قبل عرض الواجهات لتعمل في جميعها) ---
...
if load_percentage > 95:
    system_warning = "⚠️ المحولة في خطر الإنهيار! تم تفعيل الفصل الآلي لـ (قطاع D - السكني) للحفاظ على (قطاع C - المستشفى)."
    p_trans_total -= 40 # فصل افتراضي
    load_percentage = (p_trans_total / max_capacity) * 100

# --- 4. الأبواب (Tabs) لتنظيم العرض ---
...
with tab1:
    ...
    unbalance = (max(abs(ir - (ir+is_+it)/3), abs(is_ - (ir+is_+it)/3), abs(it - (ir+is_+it)/3)) / ((ir+is_+it)/3)) * 100
...
# --- 5. حلقة التحديث الآلي ---
# هذا الجزء هو ما يجعل الموقع يتحدث تلقائياً بدون تعليق الواجهات الأخرى
if live_update:
    time.sleep(2)
    st.rerun()
